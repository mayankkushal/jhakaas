...
def get_users_router(
    ...
    after_update: Optional[Callable[[models.UD,
                                     Dict[str, Any], Request], None]] = None,
        ...
        for field, value in update_dict.items():
            if field == "password":
                if validate_password:
                    await validate_password(value, user)
                hashed_password = get_password_hash(value)
                user.hashed_password = hashed_password
            elif field == 'email' and value == user.email:
                # if the email field is not changed, ignore it
                continue
            else:
                if field == 'email' and not _check_unique_email(user):
