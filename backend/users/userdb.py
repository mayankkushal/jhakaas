...
class MongoDBUserDatabase(BaseUserDatabase[UD]):
    ...
    async def get_all(self, as_dict=False) -> Optional[UD]:
        users = self.collection.find()
        if as_dict:
            return [encode_document(user) async for user in users]
        return [self.user_db_model(**user) async for user in users]
